...
In the figure, the print sharing PC has a Universal Serial Bus (USB cable) connection to the printer and a network connection, using a network interface card (NIC), to the file sharing PC.

A **NIC** is a part of a computer that lets it connect to a network (like the internet or another computer).
